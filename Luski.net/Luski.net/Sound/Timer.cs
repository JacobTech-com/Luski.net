...
namespace Luski.net.Sound
{
	public class QueueTimer
	{
		...
		public QueueTimer()
		{
			m_DelegateTimerProc = new global::Luski.net.Sound.Win32.DelegateTimerProc(OnTimer);
		}
		...
		private global::Luski.net.Sound.Win32.DelegateTimerProc m_DelegateTimerProc;
		...
		public static void SetBestResolution()
		{
			...
			global::Luski.net.Sound.Win32.TimeCaps tc = new global::Luski.net.Sound.Win32.TimeCaps();
			global::Luski.net.Sound.Win32.TimeGetDevCaps(ref tc, (uint)Marshal.SizeOf(typeof(global::Luski.net.Sound.Win32.TimeCaps)));
			uint resolution = Math.Max(tc.wPeriodMin, 0);

			//QueueTimer Resolution setzen
			global::Luski.net.Sound.Win32.TimeBeginPeriod(resolution);
		}
		/// <summary>
		/// ResetResolution
		/// </summary>
		...
		public void Start(uint milliseconds, uint dueTimeInMilliseconds)
		{
			...
			//QueueTimer Resolution setzen
			global::Luski.net.Sound.Win32.TimeBeginPeriod(m_ResolutionInMilliseconds);

			//QueueTimer Queue erstellen
			m_HandleTimerQueue = global::Luski.net.Sound.Win32.CreateTimerQueue();
			m_GCHandleTimerQueue = GCHandle.Alloc(m_HandleTimerQueue);

			//Versuche QueueTimer zu starten
			bool resultCreate = global::Luski.net.Sound.Win32.CreateTimerQueueTimer(out m_HandleTimer, m_HandleTimerQueue, m_DelegateTimerProc, IntPtr.Zero, dueTimeInMilliseconds, m_Milliseconds, global::Luski.net.Sound.Win32.WT_EXECUTEINTIMERTHREAD);
			if (resultCreate)
			{
		...
		public void Stop()
		{
			...
			{
				//QueueTimer beenden
				global::Luski.net.Sound.Win32.DeleteTimerQueueTimer(IntPtr.Zero, m_HandleTimer, IntPtr.Zero);
				//QueueTimer Resolution beenden
				global::Luski.net.Sound.Win32.TimeEndPeriod(m_ResolutionInMilliseconds);

				//QueueTimer Queue löschen
				if (m_HandleTimerQueue != IntPtr.Zero)
				{
					global::Luski.net.Sound.Win32.DeleteTimerQueue(m_HandleTimerQueue);
				}
	...
	public class EventTimer
	{
		...
		public EventTimer()
		{
			m_DelegateTimeEvent = new global::Luski.net.Sound.Win32.TimerEventHandler(OnTimer);
		}
		...
		//Delegates bzw. Events
		private global::Luski.net.Sound.Win32.TimerEventHandler m_DelegateTimeEvent;
		public delegate void DelegateTimerTick();
		public event DelegateTimerTick TimerTick;
		...
		public static void ResetResolution()
		{
			...
			//QueueTimer Resolution deaktivieren
			global::Luski.net.Sound.Win32.TimeEndPeriod(resolution);
		}
		/// <summary>
		/// Start
		/// </summary>
		...
		public void Start(uint milliseconds, uint dueTimeInMilliseconds)
		{
			...
			m_TimerId = global::Luski.net.Sound.Win32.TimeSetEvent(m_Milliseconds, m_ResolutionInMilliseconds, m_DelegateTimeEvent, ref m_UserData, (UInt32)Win32.TIME_PERIODIC);
			if (m_TimerId > 0)
			{
				//Handle im Speicher halten
		...
		public void Stop()
		{
			...
			{
				//Timer beenden
				global::Luski.net.Sound.Win32.TimeKillEvent(m_TimerId);
